...
export const getRemarkList = (params) => async (dispatch) => {
    dispatch({ type: GET_REMARK_LIST_REQUEST });
    try {
        const response = await fetchRemarkListApi(params);
        dispatch({ type: GET_REMARK_LIST_SUCCESS, payload: response.data });
        return response.data; // 👈 return response so caller can await it
    } catch (error) {
        dispatch({ type: GET_REMARK_LIST_FAILURE, payload: error.message });
        throw error; // 👈 throw so caller can catch it
    }
};
