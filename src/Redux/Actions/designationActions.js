import { createDesignation, deleteDesignationApi, fetchDesignationDetails, fetchDesignationList, fetchEmpDesignationDetails, updateDesgnStatus } from "../../services/designation";
import { ADD_DESIGNATION_FAILURE, ADD_DESIGNATION_REQUEST, ADD_DESIGNATION_SUCCESS, DELETE_DESIGNATION_FAILURE, DELETE_DESIGNATION_REQUEST, DELETE_DESIGNATION_SUCCESS, GET_DESIGNATION_DETAIL_FAILURE, GET_DESIGNATION_DETAIL_REQUEST, GET_DESIGNATION_DETAIL_SUCCESS, GET_DESIGNATION_LIST_FAILURE, GET_DESIGNATION_LIST_REQUEST, GET_DESIGNATION_LIST_SUCCESS, GET_EMPLOYEE_DESIGNATION_DETAIL_FAILURE, GET_EMPLOYEE_DESIGNATION_DETAIL_REQUEST, GET_EMPLOYEE_DESIGNATION_DETAIL_SUCCESS, UPDATE_DESIGNATION_STATUS_FAILURE, UPDATE_DESIGNATION_STATUS_REQUEST, UPDATE_DESIGNATION_STATUS_SUCCESS } from "../Constants/designationConstants";
...
export const updateDesignationStatus = (params) => async (dispatch) => {
    try {
        dispatch({ type: UPDATE_DESIGNATION_STATUS_REQUEST });
        const { data } = await updateDesgnStatus(params)
        const { message, success } = data;
        dispatch({
            type: UPDATE_DESIGNATION_STATUS_SUCCESS,
            payload: data
        });
        if (success) {
        ...
        toast.error(error.message)
        dispatch({ type: UPDATE_DESIGNATION_STATUS_FAILURE, payload: error.message });
    }
};