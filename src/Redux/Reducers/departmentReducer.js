...
export const updateDeptStatusReducer = (state = initialState, action) => {
    switch (action.type) {
        case UPDATE_DEPARTMENT_STATUS_REQUEST:
            return { ...state, loading: true, error: null };
        case UPDATE_DEPARTMENT_STATUS_SUCCESS:
            return { ...state, loading: false, error: null, data: action.payload }
        case UPDATE_DEPARTMENT_STATUS_FAILURE:
            return { ...state, loading: false, error: action.payload }
        default: return state;
    }
}