...
export const FileDetails = () => {
    ...
    useEffect(() => {
        const path = location.pathname;
        if (path.includes("/add-employee-file")) {
            setViewMode("add");
        } else if (path.includes("/edit-employee-file")) {
            setViewMode("edit");
        }
        else if (path.includes("/employee-file-details")) {
            setViewMode("detail");
        }
    ...
    const handleEditClick = () => navigate(`/edit-employee-file/${id}`);
    ...
    const renderHeader = () =>
        viewMode === "add"
            ? "Add New Employee File"
            : viewMode === "edit"
                ? "Edit Employee File Details"
    ...
    const renderHeaderInfo = () =>
        viewMode === "add"
            ? "You're just one step away from adding the New Employee File!"
            : viewMode === "edit"
                ? "Here’s the information about the Employee File Details you’ve filled."
                : "Check out Employee File information!";
    ...
    return (
        <div className="fileDeatilsMain">
            <button onClick={() => navigate(`${viewMode == 'edit' ? `/employee-file-details/${id}` : '/employee-file-list'}`)} className="close_nav header_close">Close</button>
                    ...
                    <div className="dept-page-right-panel">
                        <div className="dept-page-cover-section">
                            <div className="dept-page-basic-info-section dept-page-basic-info-section_2">
                                ...
                                <p className="dept-page-subtitle">{viewMode !== "detail" ? "Please Provide" : ''} Files Basic Details Below.</p>
                            </div>
                            {/* {viewMode !== "detail" ? */}
                            <StatusDropdown
                                options={employeeFilesStatusOptions
                            ...
                            {/* :
                                <div className="status-dropdown">
                                    <div className={`status-label dropdown-trigger`}>
                                        {employeeFilesStatusOptions?.filter((item) => item?.id == formData?.status)?.[0]?.label}
                                    </div>
                                </div>
                            } */}
                        </div>
