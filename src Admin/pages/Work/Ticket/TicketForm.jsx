...
export const TicketForm = ({ viewMode, formData, setFormData, handleSearch }) => {
    ...
    const handleSaveOrUpdate = async (e) => {
        ...
        try {
            ...
            if (res?.status === 200) {
                navigate(id ? `/ticket-details/${res?.result?.id}` : `/ticket-list`);
                if (id) dispatch(getTicketDetails({ id }));
            }
        }
        catch (error) {
                ...
                <div className="form-grid-layout">
                    <div className="dept-page-input-group">
                        ...
                        <label className={!isDetailView ? "color_red" : ""}>
                            Employee{!isDetailView ? <span>*</span> : ''}</label>
                        <SelectDropdown
                            ref={user_ref}
