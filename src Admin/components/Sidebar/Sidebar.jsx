...
import { menuItemsReport, moreItems } from '../Header/Header';
...
export const menuItems = [
    ...
    },
    {
        id: 'employee', label: 'Employee', icon: IdCardLanyard, path: '/employee-list', submenu: [
            ...
            { label: 'Employee Onboarding', BreadL: 'All Employee list', path: '/employee-list', detail: '/employee-details',BreadD:'Employee ', add: '/add-employee', BreadA: "Add New Employee", edit: '/edit-employee',BreadE:"  Employee" },
        ]
    },
    {
...
const Sidebar = ({ isExpanded, setIsExpanded }) => {
        ...
        for (const item of allItems) {
            if (item.submenu) {
                for (const sub of item.submenu) {
                    const pathsToCheck = [sub.path, sub.detail, sub.add, sub.edit].filter(Boolean); // सभी संभव पाथ को एक एरे में लें

                    for (const p of pathsToCheck) {
                        if (currentPath.startsWith(p) && p.length > longestMatchPath.length) {
                ...
                if (currentPath.startsWith(item.path) && item.path.length > longestMatchPath.length) {
                    foundParentId = item.id;
                    longestMatchPath = item.path;
                }
            }
        }
    ...
    // A simple helper function to check for path matches
    ...
    const NavItem = ({ item, moreItems = [] }) => {
        const location = useLocation();
        const currentPath = location.pathname;
        // console.log("item.id", item.id);

        // if (item.submenu && item.submenu.length > 0) {
        //     console.log("first submenu path", item.submenu[0].path);
        // } else {
        //     console.log("no submenu for", item.id);
        // }
        ...
        const isActive =
            ...
            (item.detail && isPathMatch(item.detail, currentPath)) ||
            (item.edit && isPathMatch(item.edit, currentPath));
        // ✅ New condition for Reports
        ...
        const handleParentClick = () => {
            ...
            if (!hasSubmenu) {
                ...
                (sub.edit && location.pathname.startsWith(sub.edit))
            )) {
                    ...
                    <ul className={`submenu ${isSubmenuOpen ? 'open' : ''} submenu_${item.submenu.length}`}>
                        {item.submenu.map(subItem => {
                            const subIsActive =
                                ...
                                (subItem.edit && location.pathname.startsWith(subItem.edit)) ||
                                location.pathname.startsWith(subItem.path || subItem.list);
    ...
    return (
        <aside className={`sidebar ${isExpanded ? 'expanded' : 'collapsed'}`} ref={settingRef}>
            ...
            <nav className="sidebar-nav" >
                ...
                <ul>{moreItems?.map(item => <NavItem key={item.id} moreItems={moreItems} item={item} />)}</ul>
            </nav>
