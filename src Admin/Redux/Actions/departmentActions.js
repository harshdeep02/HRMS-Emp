import { createDepartment, fetchDepartmentDetails, fetchDepartmentList, deleteDepartmentApi, fetchEmpDepartmentDetails, fetchProjectDepartmentDetails, updateDeptStatus } from "../../services/department";
import {
...
export const updateDepartmentStatus = (params) => async (dispatch) => {
    try {
        dispatch({ type: UPDATE_DEPARTMENT_STATUS_REQUEST });
        const { data } = await updateDeptStatus(params)
        const { message, success } = data;
        dispatch({
        ...
        toast.error(error.message)
        dispatch({ type: UPDATE_DEPARTMENT_STATUS_FAILURE, payload: error.message });
    }
};
